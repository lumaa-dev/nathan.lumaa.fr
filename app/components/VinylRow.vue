			...
			<div class="section">
				<div class="txt">
					<motion.a
						:href="vinyl.stream"
						class="name inline"
						:initial="{ fontVariationSettings: '\'wght\' 650, \'wdth\' 80' }"
						:transition="{
							default: { type: 'spring', duration: 0.85, bounce: 0.5 },
						}"
						:whileHover="{
							fontVariationSettings: '\'wght\' 200, \'wdth\' 140',
						}"
						>{{ vinyl.name }}</motion.a
					>
					...
					<p class="date" v-if="!!(vinyl.ownDate ?? vinyl.wishDate)">{{
						new Date(vinyl.ownDate ?? vinyl.wishDate).toLocaleDateString("fr")
					}}</p>
				</div>
			</div>
		</span>
		<span class="h">
			<span class="actions">
				...
				<template v-if="vinyl.available">
					<motion.a
						:href="vinyl.available.official"
						v-if="vinyl.available.official"
						:initial="{
							scale: 1.0,
							boxShadow: '0 0 0px var(--brand)',
							zIndex: 1,
						}"
						:transition="{
							default: { type: 'spring', duration: 0.85, bounce: 0.5 },
					...
					>
					<motion.a
						:href="vinyl.available.fnac"
						v-if="vinyl.available.fnac"
						style="background: #f5b027; color: #000"
