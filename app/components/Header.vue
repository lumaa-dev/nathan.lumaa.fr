<template>
    <MotionHeader
        ...
        :initial="{ filter: `blur(15px)`, translateY: -50, opacity: 0, width: 1000 }"
        :animate="{ filter: `blur(0px)`, translateY: 0, opacity: 1, width: $props.isCompact ? 100 : 1000 }"
        :transition="{
    ...
    >
        <span class="keeper">
            <MotionA class="img" href="/">
                <MotionImg src="/assets/hero.svg" draggable="false" 
                :initial="{ filter: 'drop-shadow(0 0 0px #ffffff00)', zIndex: 1 }"
                :transition="{ default: { type: 'spring', duration: 0.85, bounce: 0.5 } }"
                :whileHover="{ filter: 'drop-shadow(0 0 7px #ffffff55)', zIndex: 999 }" />
            </MotionA>
        </span>

        <span class="central">
            <MotionA v-for="link in $data" :href="link.path" :key="link.path"
